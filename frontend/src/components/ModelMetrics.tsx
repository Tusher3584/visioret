  ...
  return (
    <div className="flex flex-col gap-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-col gap-1">
        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-50">{metric.dataset_split_label}</h2>
...
function StatTile({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex flex-col gap-1 rounded-lg bg-slate-50 p-4 dark:bg-slate-800/50">
      <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">{label}</span>
      <span className="font-mono text-2xl font-bold text-slate-900 dark:text-slate-50">{(value * 100).toFixed(1)}%</span>
    </div>
  );
}
      ...
      <table className="w-full min-w-[420px] border-collapse text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 dark:border-slate-800 dark:text-slate-500">
            ...
            <th className="py-2 pl-4 text-right">Support</th>
          </tr>
        </thead>
        <tbody>
        ...
        <table className="border-collapse text-sm">
          <thead>
            <tr>
              <th className="p-2" />
              {labels.map((label) => (
                <th key={label} className="p-2 text-center text-xs font-semibold text-slate-500 dark:text-slate-400">
                  {label}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {matrix.map((row, i) => {
              ...
              return (
                <tr key={labels[i]}>
                  <th className="p-2 text-right text-xs font-semibold text-slate-500 dark:text-slate-400">{labels[i]}</th>
                  {row.map((count, j) => {
